...
[gitea]
repository = https://riahub.example.com/api/packages/{owner}/pypi
username = {username}
password = {password}
```
...
```shell
python3 -m twine upload --repository riahub /path/to/files/*
```
...
```shell
pip install --index-url https://{username}:{password}@riahub.example.com/api/packages/{owner}/pypi/simple --no-deps {package_name}
```
...
```shell
pip install --index-url https://testuser:password123@riahub.example.com/api/packages/testuser/pypi/simple --no-deps test_package
```
