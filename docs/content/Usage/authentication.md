...
- Username attribute (optional)
  ...
  - The attribute of the user's LDAP record containing the user name. Given
    attribute value will be used for new RIA Hub account user name after first
  ...
  - This is useful when supplied login name is matched against multiple
    attributes, but only single specific attribute should be used for RIA Hub
...
- Enable user synchronization
  - This option enables a periodic task that synchronizes the RIA Hub users with
    ...
    Filter_ settings described above will limit which users can use RIA Hub and
...
- Verify group membership in LDAP (optional)
  ...
  - Example: `(|(cn=RIA Hub_users)(cn=admins))`
...
authentication.  To work with normal Linux passwords, the user running RIA Hub
must also have read access to `/etc/shadow` in order to check the validity of
the account when logging in using a public key.

**Note**: If a user has added SSH public keys into RIA Hub, the use of these
keys _may_ bypass the login check system.  Therefore, if you wish to disable a user who
authenticates with PAM, you _should_ also manually disable the account in RIA Hub using the
...
1. Fill out the field as follows:
    ...
    field is set to `mail.com`, then RIA Hub will expect the `user email` field
...
user will log into the RIA Hub web interface as `gituser` and not `gituser@mail.com`

## SMTP (Simple Mail Transfer Protocol)

This option allows RIA Hub to log in to an SMTP host as a RIA Hub user. To
...
- Allowed Domains
  ...
  - Example: `RIA Hub.io,mydomain.com,mydomain2.com`
...
- In order to log in to RIA Hub using FreeIPA credentials, a bind account needs to
  be created for RIA Hub:

- On the FreeIPA server, create a `riahub.ldif` file, replacing `dc=example,dc=com`
  with your DN, and provide an appropriately secure password:

  ```sh
  dn: uid=riahub,cn=sysaccounts,cn=etc,dc=example,dc=com
  changetype: add
  objectclass: account
...
- Import the LDIF (change localhost to an IPA server if needed). A prompt for
  ...
  "cn=Directory Manager" -W -f riahub.ldif
  ```

- Add an IPA group for riahub_users :

  ```sh
  ipa group-add --desc="RIA Hub Users" riahub_users
  ```

- Note: For errors about IPA credentials, run `kinit admin` and provide the
  domain admin account password.

- Log in to RIA Hub as an Administrator and click on "Authentication" under Admin Panel.
...
RIA Hub supports SPNEGO single sign-on authentication (the scheme defined by RFC4559) for the web part of the server via the Security Support Provider Interface (SSPI) built in Windows. SSPI works only in Windows environments - when both the server and the clients are running Windows.

Before activating SSPI single sign-on authentication (SSO) you have to prepare your environment:

- Create a separate user account in active directory, under which the `riahub.exe` process will be running (eg. `user` under domain `domain.local`):

- Create a service principal name for the host where `riahub.exe` is running with class `HTTP`:
...
- Start the web server (`riahub.exe web`)
...
- Sign in to a client computer in the same domain with any domain user (client computer, different from the server running `riahub.exe`)
...
- Start Chrome or Edge and navigate to the FQDN URL of RIA Hub (eg. `http://host.domain.local:3000`)
...
- If it does not work, make sure that:
  - You are not running the web browser on the same server where RIA Hub is running. You should be running the web browser on a domain joined computer (client) that is different from the server. If both the client and server are running on the same computer NTLM will be preferred over Kerberos.
...
RIA Hub supports Reverse Proxy Header authentication, it will read headers as a trusted login user name or user email address. This hasn't been enabled by default, you can enable it with
