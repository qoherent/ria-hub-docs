...
RIA Hub supports these methods of API authentication:
...
RIA Hub parses queries and headers to find the token in
...
An example of the header is `X-RIA Hub-OTP: 123456` where `123456`
is where you'd place the code from your authenticator.
Here is how the request would look like in curl:

```sh
$ curl -H "X-RIA Hub-OTP: 123456" --url https://yourusername:yourpassword@gitea.your.host/api/v1/users/yourusername/tokens
```

You can also create an API key token via your RIA Hub installation's web
interface: `Settings | Applications | Generate New Token`.

## OAuth2 Provider

Access tokens obtained from RIA Hub's [OAuth2 provider](development/oauth2-provider.md) are accepted by these methods:
...
For historical reasons, RIA Hub needs the word `token` included before
...
or on the
[RIA Hub demo instance](https://try.gitea.io/api/swagger)
